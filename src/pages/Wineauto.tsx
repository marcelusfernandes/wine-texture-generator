...
const Wineauto = () => {
  ...
  // Função para lidar com os dados processados do CSV
  const handleCsvProcessed = (processedItems: { name: string; imageUrl: string | null; wineInfo: WineInfo }[]) => {
    const newWineItems: WineItem[] = processedItems
      .filter(item => item.imageUrl) // Filtra apenas itens com URL
      .map(item => ({
        id: Math.random().toString(36).substr(2, 9),
        imageUrl: `http://localhost:3000/proxy?url=${encodeURIComponent(item.imageUrl!)}`,
        wineInfo: item.wineInfo,
        filename: `${item.name.toLowerCase().replace(/\s+/g, '-')}.${
          item.wineInfo.type.toLowerCase().replace(/\s+/g, '-')}.${
          item.wineInfo.origin.toLowerCase().replace(/\s+/g, '-')}.${
          item.wineInfo.taste.toLowerCase().replace(/\s+/g, '-')}.${
          item.wineInfo.corkType.toLowerCase().replace(/\s+/g, '-')}.png`
      }));

    setWineItems(prev => [...prev, ...newWineItems]);
    toast.success(`${newWineItems.length} rótulos carregados com sucesso`);
  };
      ...
      <div className="max-w-7xl mx-auto">
        <header className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold tracking-tight">Gerador Automático de Rótulos</h1>
          </div>
          <Button
        ...
        {/* Componente BatchUpload no topo */}
        <div className="mb-8">
          <BatchUpload onLabelsProcessed={handleCsvProcessed} />
        </div>
